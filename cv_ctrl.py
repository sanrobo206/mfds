...
class OpencvFuncs():
    ...
    def __init__(self, project_path, base_ctrl, lidar_ctrl=None, gimbal_ctrl=None):
        self.base_ctrl = base_ctrl
        self.lidar_ctrl = lidar_ctrl
    ...
    def osd_render(self, osd_frame):
        ...
        # render lidar data (RPLidar)
        if self.lidar_ctrl:
            scan_data = self.lidar_ctrl.get_latest_scan()
            if scan_data:
                lidar_points = []
                # Scan format: (quality, angle, distance)
                for quality, angle, distance in scan_data:
                    if distance > 0:
                        # Convert polar to cartesian
                        # Angle in rplidar is 0-360 clockwise usually
                        # Adjust scaling factor (0.05) as needed for visualization zoom
                        angle_rad = np.radians(angle)
                        lidar_x = int(distance * np.cos(angle_rad) * 0.05) + 320
                        lidar_y = int(distance * np.sin(angle_rad) * 0.05) + 240
                        lidar_points.append((lidar_x, lidar_y))

                for lidar_point in lidar_points:
                    # Draw red dots for lidar
                    cv2.circle(osd_frame, lidar_point, 2, (0, 0, 255), -1)

        # render lidar data (Legacy YDLidar)
        lidar_points = []
    ...
    def gimbal_track(self, fx, fy, gx, gy, iterate):
        ...
        if self.gimbal_ctrl:
            self.gimbal_ctrl.set_angles(self.pan_angle, self.tilt_angle)
        else:
