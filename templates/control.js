...
var steer_left, steer_right;
var lidar_on, lidar_off;
var armZ, armR, armE;
...
fetch('/config')
  ...
  .then(yamlText => {
    try {
      ...
      steer_left = yamlObject.code.steer_left;
      steer_right = yamlObject.code.steer_right;
      lidar_on = yamlObject.code.lidar_on;
      lidar_off = yamlObject.code.lidar_off;
      ...
      ir_temp     = yamlObject.fb.ir_temp;

      if (robot_name) {
        document.title = "MFDS Web";
      }
    } catch (e) {
    ...
    try {
        ...
        if (dtIco && dTypeBtn) {
            ...
            if (data[detect_type] == cv_none) {
                ...
                if(advCBtn) advCBtn.getElementsByTagName("button")[2].classList.add("ctl_btn_active");
            } else if (data[detect_type] == cv_objs) {
                if(advFBtn) advFBtn.getElementsByTagName("button")[0].classList.add("ctl_btn_active");
            } else if (data[detect_type] == cv_clor) {
                if(advFBtn) advFBtn.getElementsByTagName("button")[1].classList.add("ctl_btn_active");
            } else if (data[detect_type] == mp_hand) {
                if(advFBtn) advFBtn.getElementsByTagName("button")[2].classList.add("ctl_btn_active");
            } else if (data[detect_type] == mp_face) {
                if(mpBtn) mpBtn.getElementsByTagName("button")[0].classList.add("ctl_btn_active");
            } else if (data[detect_type] == mp_pose) {
                if(mpBtn) mpBtn.getElementsByTagName("button")[1].classList.add("ctl_btn_active");
            }
        }
        ...
        var irData = data[ir_temp];
        if (irData) {
             document.getElementById("ir_amb_c").innerHTML = irData.ambient_c.toFixed(1) + "°C";
             document.getElementById("ir_obj_c").innerHTML = irData.object_c.toFixed(1) + "°C";
             document.getElementById("ir_amb_f").innerHTML = irData.ambient_f.toFixed(1) + "°F";
             document.getElementById("ir_obj_f").innerHTML = irData.object_f.toFixed(1) + "°F";
        }
...
function vkPress(keyCode, isDown) {
    if (isInputFocused) {
        return;
    }
    // Prevent default browser actions for some keys if needed, 
    // but here we just process logic.
    
    var key = keyMap[keyCode];
    var moveKey = moveKeyMap[keyCode];
    var val = isDown ? 1 : 0;
    
    if (key) {
        // Only update if state changed to avoid repeated firing if not needed
        // but cmdProcess might handle repetition.
        // For buttons like R (toggle), sending 1 then 0 is correct.
        if (ctrl_buttons[key] !== val) {
            updateButton(key, val);
            cmdProcess();
        }
    } else if (moveKey) {
        if (move_buttons[moveKey] !== val) {
            updateMoveButton(moveKey, val);
            moveProcess();
        }
    }
}
