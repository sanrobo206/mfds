...
if is_raspberry_pi5():
    ...
    # Use symlink for Primary Base
    base = BaseController('/dev/serial0', 115200)

try:
    # Use symlink for Secondary Base
    base2 = BaseController('/dev/base_secondary', 115200)
    print("Secondary base controller connected on /dev/base_secondary")
except Exception as e:
    base2 = None
    print(f"Failed to connect to secondary base controller: {e}")
...
# Add the directory containing steer_ctrl to path
sys.path.append(os.path.join(thisPath, 'mfds/steer'))
sys.path.append(os.path.join(thisPath, 'mfds/irtemp'))
sys.path.append(os.path.join(thisPath, 'mfds/lidar'))
sys.path.append(os.path.join(thisPath, 'mfds/gimbal'))

try:
    from steer_ctrl import SteerController
except ImportError:
    SteerController = None
    print("Could not import SteerController")

try:
    from gimbal_ctrl import GimbalController
except ImportError:
    GimbalController = None
    print("Could not import GimbalController")

try:
    from irtemp_read import IRTempReader
except ImportError:
    IRTempReader = None
    print("Could not import IRTempReader")

try:
    from rplidar_ctrl import LidarController
except ImportError:
    LidarController = None
    print("Could not import LidarController")
...
if SteerController:
    # Use symlink for Steer
    steer_controller = SteerController(port="/dev/steer")

# Gimbal Controller
gimbal_controller = None
if GimbalController:
    # Use symlink for Gimbal
    try:
        gimbal_controller = GimbalController(port="/dev/gimbal")
    except:
        gimbal_controller = None
...
if IRTempReader:
    try:
        # Defaults to /dev/irtemp with fallback to ttyUSB1 inside the class
        ir_temp_reader = IRTempReader(port="/dev/irtemp")
    except:
        ir_temp_reader = None

# Lidar Controller
lidar_controller = None
try:
    if LidarController:
        try:
            # Defaults to /dev/rplidar with fallback to ttyUSB0 inside the class
            lidar_controller = LidarController(port="/dev/rplidar")
        except:
            lidar_controller = None
except NameError:
    lidar_controller = None
...
# Camera funcs
cvf = cv_ctrl.OpencvFuncs(thisPath, base, lidar_controller, gimbal_controller)
...
cmd_actions = {
    ...
    f['code']['steer_left']: lambda: steer_controller.step_left(5) if steer_controller else None,
    f['code']['steer_right']: lambda: steer_controller.step_right(5) if steer_controller else None,

    f['code']['lidar_on']: lambda: (print("Lidar ON command"), lidar_controller.start_scan() if lidar_controller else print("No Lidar Controller")),
    f['code']['lidar_off']: lambda: (print("Lidar OFF command"), lidar_controller.stop_scan() if lidar_controller else print("No Lidar Controller"))
}
...
def handle_socket_json(json):
    try:
        # Intercept Gimbal Control (133)
        if json.get('T') == 133 and gimbal_controller:
            # {"T":133,"X":pan,"Y":tilt,"SPD":...,"ACC":...}
            gimbal_controller.set_angles(float(json.get('X', 0)), float(json.get('Y', 0)))
            return # Skip sending to base

        base.base_json_ctrl(json)
        if json.get('T') == 1:
            print(f"Motion command received: {json}")
            
            if base2:
                # Forward everything, even stops, to ensure safety
                base2.base_json_ctrl(json)
    except Exception as e:
...
def update_data_websocket_single():
    ...
    try:
        socket_data = {
            ...
            f['fb']['ir_temp']:     ir_temp_reader.get_temp() if ir_temp_reader else {"ambient_c": 0, "object_c": 0, "ambient_f": 0, "object_f": 0}
        }
